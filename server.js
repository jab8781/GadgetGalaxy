...
// DELETE route for product removal
app.delete('/api/products', async (req, res) => {
    try {
        const { productIds } = req.body;

        if (!productIds || !Array.isArray(productIds)) {
            return res.status(400).json({ message: 'Invalid product IDs format. Expecting an array.' });
        }

        const db = client.db('team5DB');
        const collection = db.collection('products');

        // Ensure the IDs are strings for querying
...
// PUT route for product quantity update
app.put('/api/products/quantity', async (req, res) => {
    try {
